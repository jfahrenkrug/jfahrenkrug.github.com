...
})();ImageSearch=SC.Application.create({NAMESPACE:"ImageSearch",VERSION:"0.1.0",store:SC.Store.create({commitRecordsAutomatically:YES}).from("ImageSearch.GoogleImageDataSource")});
ImageSearch.currentImageController=SC.ObjectController.create({contentBinding:SC.Binding.single("ImageSearch.imagesController.selection"),});
ImageSearch.imagesController=SC.ArrayController.create({searchTerm:null,summary:function(){var a=this.get("length"),b;
if(a&&a>0){b=a===1?"1 image":"%@ images".fmt(a)}else{b="No images"}return b}.property("length").cacheable(),performSearch:function(){var b=SC.Query.remote(ImageSearch.GoogleImage,{query:this.get("searchTerm")});
var a=ImageSearch.store.find(b);ImageSearch.imagesController.set("content",a)},showPopover:function(a){console.log("view = "+a);
var b=SC.PickerPane.create({layout:{width:200,height:500},contentView:SC.View.design({layout:{top:0,bottom:0,width:200},childViews:"textField scrollView".w(),textField:SC.TextFieldView.design({layout:{top:2,height:24,left:5,right:5},controlSize:SC.LARGE_CONTROL_SIZE,fontWeight:SC.BOLD_WEIGHT,hint:"type your search here",valueBinding:"ImageSearch.imagesController.searchTerm",target:"ImageSearch.imagesController",action:"performSearch",keyDown:function(c){arguments.callee.base.apply(this,arguments);
if(c.keyCode===13){ImageSearch.imagesController.performSearch();return YES}else{return NO
}}}),scrollView:SC.ScrollView.design({hasHorizontalScroller:NO,layout:{top:28,bottom:0,left:5,right:5},backgroundColor:"white",contentView:SC.ListView.design({contentBinding:"ImageSearch.imagesController.arrangedObjects",selectionBinding:"ImageSearch.imagesController.selection",contentValueKey:"content",contentIconKey:"tbUrl",exampleView:ImageSearch.ImageThumbListItemView,hasContentIcon:YES,escapeHTML:NO,rowHeight:70,})})})});
b.popup(a,SC.PICKER_FIXED)},});ImageSearch.GoogleImageDataSource=SC.DataSource.extend({fetch:function(b,c,d){var a=this;
console.log("in fetch");if(SC.buildMode==="debug"){SC.Request.getUrl("ajax/services/search/images?v=1.0&rsz=large&imgtype=photo&q="+c.query).json().notify(this,"didFetchImages",b,c).send()
}else{$.getJSON("http://ajax.googleapis.com/ajax/services/search/images?v=1.0&rsz=large&imgtype=photo&q="+c.query+"&callback=?",null,function(e,g,f){SC.run(function(){var h=SC.Response.create({request:null,body:e,status:g});
a.didFetchImages(h,b,c)})})}return YES},didFetchImages:function(b,a,e){console.log("in didfetch");
var d;if(SC.ok(b)){console.log("response ok");d=b.get("body").responseData.results;
var c=a.loadRecords(ImageSearch.GoogleImage,d);a.loadQueryResults(e,c);a.dataSourceDidFetchQuery(e)
...
}}}}})})(window);ImageSearch.GoogleImage=SC.Record.extend({primaryKey:"imageId",content:SC.Record.attr(String),contentNoFormatting:SC.Record.attr(String),height:SC.Record.attr(Number),imageId:SC.Record.attr(String),originalContextUrl:SC.Record.attr(String),tbHeight:SC.Record.attr(Number),tbUrl:SC.Record.attr(String),tbWidth:SC.Record.attr(Number),title:SC.Record.attr(String),titleNoFormatting:SC.Record.attr(String),unescapedUrl:SC.Record.attr(String),url:SC.Record.attr(String),visibleUrl:SC.Record.attr(String),width:SC.Record.attr(Number)});
ImageSearch.AwesomeSplitView=SC.SplitView.extend({collapseTopLeftView:function(a){if(a){return this.collapseView(this._topLeftView)
}else{return this.uncollapseView(this._topLeftView)}},collapseBottomRightView:function(a){if(a){return this.collapseView(this._bottomRightView)
}else{return this.uncollapseView(this._bottomRightView)}},collapseView:function(a){if(a!==this._topLeftView&&a!==this._bottomRightView){return NO
}this._cacheCurrentDimensions();var b=a.get("isCollapsed")||NO;if(!b&&this.canCollapseView(a)){this._uncollapsedThickness=this.thicknessForView(a);
this._uncollapsedDividerThickness=this.get("dividerThickness");if(a===this._topLeftView){this._updateTopLeftThickness(this.topLeftThickness()*-1)
}else{this._updateBottomRightThickness(this.bottomRightThickness()*-1)}if(!a.get("isCollapsed")){this._uncollapsedThickness=null;
return NO}this.set("dividerThickness",0);this.updateChildLayout();return YES}return NO
},uncollapseView:function(a){if(a!==this._topLeftView&&a!==this._bottomRightView){return NO
}var b=a.get("isCollapsed")||NO;if(b){this.set("dividerThickness",(this._uncollapsedDividerThickness||7));
this._cacheCurrentDimensions();if(a===this._topLeftView){this._updateTopLeftThickness(this._uncollapsedThickness)
}else{this._updateBottomRightThickness(this._uncollapsedThickness)}a._uncollapsedThickness=null;
this.updateChildLayout()}return true},_cacheCurrentDimensions:function(){this._topLeftView=this.get("topLeftView");
...
this._layoutDirection=this.get("layoutDirection")},});ImageSearch.ImageThumbListItemView=SC.ListItemView.extend({escapeHTML:NO,classNames:["sc-list-item-view"],});
ImageSearch.ResizableImageView=SC.ImageView.extend({currentImageSize:{width:0,height:0},imageDidLoad:function(a,b){arguments.callee.base.apply(this,arguments);
if(SC.$ok(b)){console.log("image did load"+b);this.currentImageSize.width=b.width;
this.currentImageSize.height=b.height;this._aspectSizeToFit()}},sizeToFit:function(){this._aspectSizeToFit()
}.observes(".parentView.frame"),_aspectSizeToFit:function(){var g=this.parentView.get("frame");
theGlobalF=g;console.log(g);var b=this.currentImageSize.width;var d=this.currentImageSize.height;
var e=b;var a=d;if(g.height>=(d*(g.width/b))){e=g.width;a=d*(g.width/b)}else{a=g.height;
e=b*(g.height/d)}if(this.get("layer")){var c=this.get("layer").style;c.width=e+"px";
c.height=a+"px";c.top=((g.height-a)/2)+"px";c.left=((g.width-e)/2)+"px"}}});ImageSearch.DEFAULT_DIVIDER_WIDTH=0.8;
ImageSearch.mainPage=SC.Page.design({mainPane:SC.MainPane.design({childViews:"middleView topView bottomView".w(),topView:SC.ToolbarView.design({layout:{top:0,left:0,right:0,height:36},childViews:"imagesButton labelView".w(),anchorLocation:SC.ANCHOR_TOP,imagesButton:SC.ButtonView.design({layout:{centerY:0,height:24,left:8,width:200},title:"Pick an Image",action:"showPopover",target:"ImageSearch.imagesController"}),labelView:SC.LabelView.design({layout:{centerY:0,height:24,left:213,right:8},controlSize:SC.LARGE_CONTROL_SIZE,fontWeight:SC.BOLD_WEIGHT,valueBinding:"ImageSearch.currentImageController.title",escapeHTML:NO})}),middleView:ImageSearch.AwesomeSplitView.design({layout:{left:0,top:36,right:0,bottom:32},layoutDirection:SC.LAYOUT_HORIZONTAL,autoresizeBehavior:SC.RESIZE_BOTTOM_RIGHT,defaultThickness:0.8,topLeftView:SC.View.design({layout:{top:36,bottom:32,width:200},childViews:"textField scrollView".w(),textField:SC.TextFieldView.design({layout:{top:2,height:24,left:5,right:5},controlSize:SC.LARGE_CONTROL_SIZE,fontWeight:SC.BOLD_WEIGHT,hint:"type your search here",valueBinding:"ImageSearch.imagesController.searchTerm",target:"ImageSearch.imagesController",action:"performSearch",keyDown:function(a){arguments.callee.base.apply(this,arguments);
if(a.keyCode===13){ImageSearch.imagesController.performSearch();return YES}else{return NO
}}}),scrollView:SC.ScrollView.design({hasHorizontalScroller:NO,layout:{top:28,bottom:0,left:5,right:5},backgroundColor:"white",contentView:SC.ListView.design({contentBinding:"ImageSearch.imagesController.arrangedObjects",selectionBinding:"ImageSearch.imagesController.selection",contentValueKey:"content",contentIconKey:"tbUrl",exampleView:ImageSearch.ImageThumbListItemView,hasContentIcon:YES,escapeHTML:NO,rowHeight:70,})})}),topLeftMaxThickness:250,dividerView:SC.SplitDividerView.design({layout:{}}),bottomRightView:SC.ScrollView.design({layout:{left:5,top:36,right:5,bottom:32},horizontalAlign:SC.ALIGN_CENTER,verticalAlign:SC.ALIGN_CENTER,canScale:YES,contentView:ImageSearch.ResizableImageView.design({layout:{left:0,top:0,right:0,bottom:0},valueBinding:"ImageSearch.currentImageController.unescapedUrl"})})}),bottomView:SC.ToolbarView.design({layout:{bottom:0,left:0,right:0,height:32},anchorLocation:SC.ANCHOR_BOTTOM})})});
ImageSearch.main=function main(){ImageSearch.getPath("mainPage.mainPane").append();
var b=SC.Query.remote(ImageSearch.GoogleImage,{query:"ren and stimpy"});var a=ImageSearch.store.find(b);
ImageSearch.imagesController.set("content",a)};function adjustLayoutToOrientation(){var d=SC.device.get("orientation");
var b=ImageSearch.getPath("mainPage.mainPane.middleView");var a=ImageSearch.getPath("mainPage.mainPane.topView.imagesButton");
var e=ImageSearch.getPath("mainPage.mainPane.topView.labelView");var c=SC.clone(e.get("layout"));
b.collapseTopLeftView(d==="portrait");a.set("isVisible",d==="portrait");c.left=(d!=="portrait")?8:213;
e.set("layout",c)}function main(){ImageSearch.main();SC.device.addObserver("orientation",this,adjustLayoutToOrientation);
